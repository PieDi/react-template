...
function extractParamsByPath(list = []) {
  ...
  try {
    ...
    throw new Error('event trace error!');
  }
...
function traceCustomEvent(eventId, title, params = {}) {
  emit({
    eventId, title, params, eventType: 'custom', triggerTime: Date.now(),
  });
}
...
function clickCollection() {
  document.addEventListener('click', (e) => { // 点击事件
    ...
    config.title = extractTitleByTarget(target);
    config.eventId = extractDataByPath(path);
    config.params = extractParamsByPath(path);
    config.elementPath = getNodeXPath(target).slice(-128); // 长度限制128字符
    ...
    config.x = x;
    config.y = y;
    config.triggerTime = Date.now(); // 点击时间
    config.url = window.location.href; // 当前页面的url
    emit(config);
  }, true);
}
...
function dwellCollector(eventUnload) {
  ...
  window.addEventListener('beforeunload', () => { // 卸载事件
    ...
    config.triggerTime = Date.now(); // 卸载时间
    config.millisecond = Date.now() - config.entryTime; // 停留多久
    const mapping = {
    ...
    config.operateAction = mapping[type] || null;
    emit(config, true);
  }, false);
}
